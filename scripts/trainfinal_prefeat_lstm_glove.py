...
def prepareQuestionVocab( questionsFile, threshQ , gloveFile):
    ...
    glove_data=(open(gloveFile)).read().split('\n')
    glove_vocab={}
    vocab={}
    current=1
    ...
    return [ vocab, vocab_size, glove_vocab, glove_data ]


def gloveVector(word_vector, glove_vocab, vocab):
    out = np.zeros()
    sim_map = {}
    word_vector = np.array(word_vector, dtype='float16')
    for key in glove_vocab.keys():
        vector = map(float, glove_vocab[key])
        vector = np.array(vector, dtype='float16')
        sim_map[key] = np.dot(word_vector,vector)

    sim_word = sorted(sim_map.items(), key=lambda x:x[1], reverse=True).keys()[0]
    return vocab[sim_word]

######################################### Question Data ########################################################

def prepareQuestionData( vocab, questionsFile, questionsFileVal , glove_vocab, glove_data):
        ...
        for i in range(min(len(question),10)):
            if vocab.has_key(question[i]):
                ...
                for line in glove_data:
                    ...
                    if (glove_word == word):
                        glove_vector = line[1:]
                        glove_vector = map(float, glove_vector)
                        value = gloveVector(glove_vector, glove_vocab, vocab)
                        break
